...
1. **Internal (you + Fabric POC):** A realistic, daily-updating Fabric dataset to test Fabric POC end-to-end.
...
These two audiences don't conflict — they reinforce. The same repo that lets you test Fabric POC is also the best possible "Fabric developer does it right" showcase for your LinkedIn. The dataset is the byproduct; the *pipeline and developer workflow* are the story.
...
- 3–5 LinkedIn posts as you build, each showcasing a different piece (the generator, the MCP setup, the daily pipeline, Fabric POC running on top)
...
target = FabricWorkspace(
    ...
    item_type_in_scope=["Notebook", "DataPipeline", "Lakehouse", "ReportingLayer"],
)
...
│   ├── semantic_models/
│   │   └── CSNP_Retail_Model.ReportingLayer/
│   │       ├── definition/
│   │       │   ├── model.tmdl
│   │       │   ├── database.tmdl
│   │       │   ├── cultures/
...
└────────────┬────────────────────────────────────────────────────────┘
 ...
 └───────────┬────────────────────────────────────────────────────┘
             ...
             ▼
 ┌───────────────────────────────────────────────────────────────┐
 │ 7. Web activity: reframe reporting layer                        │
...
Each notebook activity runs on a 4-core / 16 GB Spark pool. The `run_manifest.json` written to `Files/pipeline_runs/{run_id}/` captures row counts, timing, DQ results — auditable, debuggable, and itself a dataset Fabric POC can point at.
...
The most important and most Fabric POC-specific layer: verify each planted insight is detectable in the generated data. If generator changes break a pattern, the demo script breaks.
...
Each test runs: generate data → run a simple aggregation → assert the pattern is present within tolerances. If Fabric POC's LLM layer can't find the pattern, either the data broke or the detection layer broke — either way, known.
...
**Goal.** Push Week 1's bronze Parquet to OneLake, transform through silver into gold, have a Direct Lake reporting layer answering DAX queries.
...
**Day 12–13.** Gold notebooks. SCD2 handling for dims, surrogate-key ledger, fact loading with late-arrival window. This is where the reporting layer work starts — TMDL files, measures, hierarchies.

**Day 14.** First end-to-end run: backfill bronze → silver → gold, open the reporting layer in Power BI, verify measures calculate correctly, run DAX queries through `executeQueries`.

**End of Week 2 deliverable.** Dev workspace has full 3-year backfill loaded, reporting layer works, canonical DAX queries return the expected values.
...
**Day 16.** Data Quality layer. Great Expectations project set up. Expectation suites for bronze and gold. Checkpoints in the pipeline. DQ results written to Delta table (the "Fabric POC monitors itself" story).
...
### Week 4 — Fabric POC integration + polish

**Goal.** Point Fabric POC at the Prod reporting layer. Generate insight dashboards. Record the demo.

**Day 22–23.** Fabric POC integration. Connect to `CSNP_Retail_Model`. Generate dashboards for each of the 14 planted patterns. Document which ones Fabric POC's pipeline catches automatically vs. which need prompting. This is the most valuable internal exercise — it tells you where Fabric POC needs work.

**Day 24.** Build the "editorial" dashboard from earlier mockup against live data. This becomes the hero screenshot.

**Day 25.** Demo Loom. 5–7 minute walkthrough: prompt → Fabric POC generates insight dashboard from live Fabric data → contrast with native Power BI equivalent. This goes to Shreyas *and* goes on LinkedIn.

**Day 26–27.** LinkedIn campaign. 3 posts across 2 weeks:
  - Post 4: "I pointed Fabric POC at a real Fabric reporting layer running daily. Here's what the output looks like." (Hero dashboard screenshot)
  - Post 5: "Most AI BI tools hallucinate insights. Fabric POC runs actual statistical analysis, uses the LLM only for narration. Here's a pattern buried in our data that it found without being told." (The Meridian Cardigan or Signature Sizing story)
...
**End of Week 4 deliverable.** Fabric POC demo for Shreyas. Public repo with stars. LinkedIn posts getting traction.
...
**Week 4 (integration).** More hands-on. This is where you sit next to Claude Code and iterate on the Fabric POC prompts, the dashboard aesthetics, the LinkedIn post wording. Less autonomous execution, more creative back-and-forth. Expect 8–10 shorter sessions.
...
A synthetic retail dataset for Microsoft Fabric, used to test Fabric POC end-to-end.
...
- fabric/semantic_models/CSNP_Retail_Model.ReportingLayer/definition/ — measures live here
```
...
5. **Fabric POC on top.** The editorial dashboard, live from Prod. "This is what AI-native analytics looks like." Your actual product gets seen.
6. **The whole thing, packaged.** "Everything is public. Fork the repo." The repo URL in the post. Long-tail inbound.

Post #5 is the one that matters most for Fabric POC commercially. Posts #1-#4 build the audience and credibility so that post #5 lands on prepared ground rather than cold.
