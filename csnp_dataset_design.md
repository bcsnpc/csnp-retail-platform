# CSNP & Co. — Semantica Test Dataset & Pipeline Design

**Version:** 1.0 · **Author:** Chiranjeevi Bhogireddy · **Date:** 21 Apr 2026
**Brand:** CSNP & Co. (fictional apparel retailer)
**Purpose:** End-to-end synthetic but realistic retail dataset for Semantica testing. Covers domain, schema, planted patterns, Fabric medallion pipeline, and daily incremental load orchestrated via Data Factory + Notebook.
...
**Brand aesthetic for demos.** Heritage-editorial — serif masthead, warm neutrals, editorial layouts. Pairs with the dashboard direction mocked earlier. In every Semantica screenshot, "CSNP & Co." appears as the data source or brand header, making demos unmistakably personal to you.
...
1. **Exercise every visual type** Semantica supports — line, bar, treemap, Sankey, funnel, cohort heatmap, map, waterfall, scatter, gauge, small multiples, Pareto, distribution, KPI cards.
...
3. **Run end-to-end in Fabric** — bronze landing in OneLake, medallion transformation, Direct Lake semantic model, daily incremental append via Data Factory pipeline. Not a static CSV dump.
...
Relationships: single-direction dim → fact, single-column keys. `dim_date` is referenced by every fact. `dim_geography` chains from both `dim_customer` and `dim_store` — tests how Semantica handles shared dimensions.
...
This is what Semantica would see if it pointed at a real Fabric Lakehouse. Silver absorbs all of it.
...
This is what the Semantica semantic model points at.
...
- RLS role `RegionManager` — restricts to assigned region, for testing security propagation through Semantica
...
Each pattern below is something Semantica's multi-agent pipeline should discover and narrate.
...
```python
# In the Fabric pipeline, call the semantic model refresh API
...
After gold load and semantic model reframe, a validation notebook runs:
...
### 8.9 Why the pipeline structure matters for Semantica

This structure isn't just a test fixture — it's also a realistic reference implementation that mirrors what Semantica's eventual customers will have. When demo'ing:

- "Semantica works with your existing medallion pipeline" — shown, not claimed
- "Semantica handles SCD2 correctly" — you can demo queries against historical snapshots
- "Semantica picks up daily changes automatically" — just wait a day, refresh the insight, watch new anomalies appear
- Daily DQ results become their own Semantica demo — point it at the `dq_results` table and generate insight dashboards about your own pipeline
...
Ships with the semantic model so every Semantica-generated visual references named measures, not ad-hoc expressions.
