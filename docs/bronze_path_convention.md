...
The `merge_to_silver` helper accepts a `target_table_fqn` parameter for this:

```python
merge_to_silver(
    source_df=silver_df,
    silver_table_path=silver_abfss_path,   # used for MERGE via DeltaTable.forPath
    target_table_fqn="CSNP_Silver.silver_dim_date",  # used for initial saveAsTable
